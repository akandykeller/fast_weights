...
with tf.Session() as sess:
    model = FWQA(batch_size, vocab_size, sentence_size, memory_size, FLAGS.embedding_size, FLAGS.num_hidden_units, FLAGS.S, FLAGS.S_Q,
                 tied_output=FLAGS.tied_output, session=sess, max_grad_norm=FLAGS.max_grad_norm, log_dir=FLAGS.log_dir)
    ...
    for t in range(1, FLAGS.epochs+1):
        ...
        if t % FLAGS.evaluation_interval == 0:
            ...
            for b_idx, start in enumerate(range(0, n_train-batch_size, batch_size)):
                ...
                q = trainQ[start:end]
                a = trainA[start:end]
                summary, pred = model.predict(s, q, a, FLAGS.eta, FLAGS.decay_lambda)
