    ...
    def _build_vars(self):
        ...
        with tf.variable_scope("fast_weights_rnn"):
            for layernum in range(self._num_layers):
                if layernum == 0:
                    W_x_shape = [self._embedding_size, self._num_hidden_units]
                else:
                    W_x_shape = [self._num_hidden_units, self._num_hidden_units]
                ...
                for scope in ["fw/multi_rnn_cell/cell_{}".format(layernum), "bw/multi_rnn_cell/cell_{}".format(layernum)]:
                    with tf.variable_scope(scope):
                        ...
                        self.W_x = tf.get_variable(name="W_x",
                                        # shape=W_x_shape,
                                        initializer=tf.random_uniform(W_x_shape,               
                                                    -np.sqrt(2.0/W_x_shape[0]), np.sqrt(2.0/W_x_shape[0])),
                                        dtype=tf.float32)

                        self.b_x = tf.get_variable(name="b_x",
                        ...
                        self.gain = tf.get_variable(name='gain',
                            # shape=[self._num_hidden_units],
                ...
                self.fw_cells_list.append(FastWeightsRNNCell(num_hidden_units=self._num_hidden_units, 
                ...
                self.bw_cells_list.append(FastWeightsRNNCell(num_hidden_units=self._num_hidden_units, 
                                                               batch_size=self._batch_size, loop_steps=self._S,
                                                               decay_rate=self.lambdas[layernum], eta=self.etas[layernum], 
                                                               dropout_keep_prob=self._keep_prob))
