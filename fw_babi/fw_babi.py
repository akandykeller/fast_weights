...
This implementation is identical to the implementation in fw_cell_babi_ml.py, which uses
the lnfw_rnn_cell.py cell implementation, but the graph is explicitly unrolled, so it 
does not use any cells. This can be used for correctness testing of the multi-layer
and cell implementations.
...
class FWQA(object):
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):
            ...
            # self.W_qm = tf.Variable(tf.random_uniform(
            #                         [2 * self._embedding_size, self._embedding_size],
            #                         -np.sqrt(2.0/self._embedding_size),
            #                         np.sqrt(2.0/self._embedding_size)),
            #                         dtype=tf.float32, name="W_qm")

            # # importance weights
            # self.w_b = tf.Variable(tf.random_uniform(
    ...
    def _inference(self, stories, queries):
        ...
        with tf.variable_scope(self._name):
            ...
            m_enc = tf.reduce_sum(m_emb * self._encoding, axis=2)

            # Concat memory with question encoding and reproject back to dim=20
            # m_enc = tf.concat([m_enc, tf.tile(q_enc, [1, self._memory_size, 1])], axis=2)
            # m_enc = tf.reshape(tf.matmul(tf.reshape(m_enc, (self._batch_size * self._memory_size, -1)), self.W_qm), (self._batch_size, self._memory_size, -1))
            
            # Create 'importance weight' for each sentence word
            # word_importances = tf.nn.softmax(tf.reduce_sum(tf.multiply(self.w_b, m_emb), axis=3), dim=2)
            # m_enc = tf.reduce_sum(tf.multiply(m_emb * self._encoding, tf.expand_dims(word_importances, -1)), axis=2)

            input_seq = tf.concat(axis=1, values=[m_enc, q_enc])

        # Create question-aware story sentence emebddings

        with tf.variable_scope("fast_weights"):
            # NOTE:inputs are batch-major
            # Process batch by time-major
            for t in range(0, input_seq.get_shape().as_list()[1]):
                self.h = tf.nn.dropout(self.h, self._keep_prob)

                # hidden state (preliminary vector)
                self.h = tf.nn.relu((tf.matmul(input_seq[:, t, :], self.W_x) + self.b_x) + tf.matmul(self.h, self.W_h))

                # Reshape h to use with a
                self.h_s = tf.reshape(self.h,
                    [self._batch_size, 1, self._num_hidden_units])

                # Create the fixed A for this time step
                self.A = tf.add(tf.scalar_mul(self._l, self.A),
                    tf.scalar_mul(self._e, tf.matmul(tf.transpose(
                        self.h_s, [0, 2, 1]), self.h_s)))

                # Loop for S steps
                S_steps = self._S if t < self._memory_size else self._S + self._S_Q
                for _ in range(S_steps):
                    self.h_s = tf.reshape(tf.matmul(input_seq[:, t, :], self.W_x) + self.b_x, tf.shape(self.h_s)) \
                                + tf.reshape(tf.matmul(self.h, self.W_h), tf.shape(self.h_s)) \
                    ...
                    self.h_s = tf.expand_dims(self.h_s, 1)

                # Reshape h_s into h
                self.h = tf.reshape(self.h_s, [self._batch_size, self._num_hidden_units])

            if self._tied_output:
                self.h_out = tf.matmul(self.h, tf.transpose(self.W_x, [1,0]))

                with tf.variable_scope(self._name):
                    logits =  tf.matmul(self.h_out, tf.transpose(self.LUT, [1,0]))
            else:
                logits = tf.matmul(self.h, self.W_softmax) + self.b_softmax
